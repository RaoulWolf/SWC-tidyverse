...
Note that we split the function into two lines. After `ggplot()`, the plus sign indicates that the function is not over yet and that the next line should be interpreted as additional layer to the preceding `ggplot()` function. In other words, when writing a `ggplot()` function spanning several lines, the `+` sign goes at the end of the line, not in the beginning.
...
```{r}
ggplot(data = gapminder) + 
  geom_jitter(mapping = aes(x = year, y = lifeExp, colour = continent))
```
...
## Part 1
ggplot(data = gapminder) + 
  geom_jitter(mapping = aes(x = continent, y = lifeExp, colour = year))

# Color by country
ggplot(data = gapminder) + 
  geom_jitter(mapping = aes(x = continent, y = lifeExp, colour = country))  
...
```{r, results="hide"}
ggplot(data = gapminder) + 
  geom_point(mapping = aes(x = gdpPercap, y = lifeExp, color = year, shape = continent, size = pop)) +
  scale_x_log10()
```
...
```{r}
ggplot(data = gapminder) + 
  geom_point(mapping = aes(x = gdpPercap, y = lifeExp), colour = "blue", size = 2, alpha = 0.2) +
  scale_x_log10()
```
...
#
ggplot(data = gapminder) + 
  geom_point(mapping = aes(x = gdpPercap, y = lifeExp, colour = year)) +
  scale_x_log10()
...
## **JUST DEMONSTRATE THIS!!! Challenge 6.** {.tabset .tabset-fade .tabset-pills}
